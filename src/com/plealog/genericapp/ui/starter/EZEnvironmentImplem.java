...
public abstract class EZEnvironmentImplem {
	...
	private static enum CONF_PNL_ALIGN {left, center};
    
    private static CONF_PNL_ALIGN _defAlign = CONF_PNL_ALIGN.left;

	private static final ResourceBundle RESOURCE_BUNDLE = ResourceBundle.getBundle(SYSTEM_BUNDLE_NAME);
     ...
     * @return the value associated to key. Returns "?" is key if not found or not defined, or
     ...
     * @param bundle a resource bundle
     * @param key a resource key
     * @return the value associated to key. Returns 0 (zero) if key is not found or not 
     ...
     * @return the value associated to key. Returns false if key is not found or not 
     * defined, or if Properties file is not found.
     */
     ...
     * @param bundle a resource bundle
     * @param key a resource key
     * @return the value associated to key. Returns Color.black if key is not found or not 
     * defined, or if Properties file is not found.
     */
    ...
    /** 
	 * Returns an Image given its name, null if not found. See
	 * this.getImageIcon() for more information.
	 */
	 ...
	 * 
	 * @param imageName an image name which just consists of name
	 *        follows by a period and an extension. Example: new.gif.
	 *        If an absolute path is required to get the image, consider
	...
	public static ImageIcon getImageIcon(String imageName){
		...
		if (imageName != null && imageName.length()>1){
			...
			//try by path
            for (i=0;i<size;i++){
                url = EZEnvironmentImplem.class.getResource(
                    _imgPaths.get(i) + imageName.trim() ); 
                if (url != null){
	...
	/** 
	 * Adds a path in the Image search path list. A 'path' is actually the name
	 * of a package that is supposed to also contain image ressources. A path
	 * has to start with '/' and has to use '/' instead of '.'. Example: you
	 * can pass in here "/my/package/" when you want to get images from the 
	 * package "my.package".
	 */
    ...
    /** 
     * Adds a Class in the Image search Class list. This is to be used when application
     * is shipped within a JAR: search path seems not to work, so use this method to add
     * a class that is located is the same directory as the image to load.
     */
	...
	public static String inputValueMessage(Component parent, String msg, String initValue){
		Object obj =JOptionPane.showInputDialog(
				...
				JOptionPane.QUESTION_MESSAGE, null, null, initValue);
